...
# ARG_OPTIONAL_SINGLE([rpi-resource],[r],[folder with files for rpi installation],[/home/vagrant/rpi-files])
# ARG_DEFAULTS_POS()
...
_arg_rpi_resource="/home/vagrant/rpi-files"
...
print_help()
{
	...
	printf '\t%s\n' "-r, --rpi-resource: folder with files for rpi installation (default: '/home/vagrant/rpi-files')"
	printf '\t%s\n' "-h, --help: Prints help"
}
...
if [ -d $_arg_share/src/firmware ]; then
    printf '# Using cache\n'
    git clone --quiet -b stable --depth=1 file://$_arg_share/src/firmware
else
    git clone --quiet -b stable --depth=1 https://github.com/raspberrypi/firmware
fi
...
# printf '# ################## #\n'
# git clone --quiet https://github.com/raspberrypi/linux
...
cd $_arg_rpi_folder
# use cache if possible
if [ -f $_arg_share/src/stage3-arm64-20200609.tar.bz2 ] && [ -f $_arg_share/src/stage3-arm64-20200609.tar.bz2.DIGESTS ]; then
    printf '# Using cache\n'
    cp $_arg_share/src/stage3-arm64-20200609.tar.bz2.DIGESTS .
    cp $_arg_share/src/stage3-arm64-20200609.tar.bz2 .
else
    wget --quiet http://distfiles.gentoo.org/experimental/arm64/stage3-arm64-20200609.tar.bz2.DIGESTS
    wget --quiet http://distfiles.gentoo.org/experimental/arm64/stage3-arm64-20200609.tar.bz2
fi
b2sum stage3-arm64-20200609.tar.bz2 | grep -q -f - stage3-arm64-20200609.tar.bz2.DIGESTS || (printf 'verify failed\n' && exit 1)
sudo tar xpf stage3-*.tar.bz2 --xattrs-include='*.*' --numeric-owner
rm stage3-arm64-20200609.tar.bz2*
...
cd $_arg_rpi_folder
# use cache if possible
if [ -f $_arg_share/src/portage-latest.tar.bz2 ] && [ -f $_arg_share/src/portage-latest.tar.bz2.md5sum ]; then
    printf '# Using cache\n'
    cp $_arg_share/src/portage-latest.tar.bz2.md5sum .
    cp $_arg_share/src/portage-latest.tar.bz2 .
else
    wget --quiet http://distfiles.gentoo.org/snapshots/portage-latest.tar.bz2.md5sum
    wget --quiet http://distfiles.gentoo.org/snapshots/portage-latest.tar.bz2
fi
cd $_arg_rpi_folder
...
sudo tar xpf portage-latest.tar.bz2 --strip-components=1 -C $_arg_rpi_folder/var/db/repos/gentoo
...
sudo cp $_arg_share/rpi/make.conf $_arg_rpi_folder/etc/portage
...
cd $_arg_rpi_resource/
# use cache if possible
if [ -d $_arg_share/src/firmware-nonfree ]; then
    printf '# Using cache\n'
    git clone --quiet file://$_arg_share/src/firmware-nonfree
else
    git clone --quiet https://github.com/RPi-Distro/firmware-nonfree
fi
...
printf '## ################## #\n'
sudo wget --quiet --directory-prefix=$_arg_rpi_folder/lib/firmware/brcm https://raw.githubusercontent.com/RPi-Distro/bluez-firmware/master/broadcom/BCM4345C0.hcd
...
cd $_arg_rpi_resource
# use cache if possible
if [ -d $_arg_share/src/tools ]; then
    printf '# Using cache\n'
    git clone --quiet file://$_arg_share/src/tools
else
    git clone --quiet https://github.com/raspberrypi/tools
fi
cd tools/armstubs
...
sudo cp $_arg_share/rpi/wpa_supplicant.conf $_arg_rpi_folder/etc/wpa_supplicant/wpa_supplicant.conf
